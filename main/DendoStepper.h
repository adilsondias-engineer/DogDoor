...
public:
     ...
     */
    esp_err_t runPos(int32_t relative,bool direction);

    /** @brief runs motor to relative position in mm
     *  @param relative number of mm to run, negative is reverse
     */
    esp_err_t runPosMm(int32_t relative,bool direction);
     ...
     */
    esp_err_t runAbs(uint32_t position, bool direction);
     ...
     *  @return ESP_OK if motor can run immediately, ESP_ERR if it is currently moving
     */
    esp_err_t runAbsMm(uint32_t position, bool direction);
