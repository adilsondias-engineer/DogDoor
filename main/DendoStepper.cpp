/*
* source : https://github.com/eldendiss/DendoStepper
*/
...
esp_err_t DendoStepper::runPos(int32_t relative,bool direction)
{
    ...
    { // we are running, we need to adjust steps accordingly, for now just stop the movement
      ...
      //  return ESP_ERR_NOT_SUPPORTED;
        ...
        stop(); //stop the motor to change direction
    ...
    if (ctrl.status == DISABLED) // if motor is disabled, enable it
        enableMotor();
    ctrl.status = ACC;
    setDir(direction); // set CCW if <0, else set CW
...
esp_err_t DendoStepper::runPosMm(int32_t relative,bool direction)
{
    if (ctrl.stepsPerMm == 0)
    {
        STEP_LOGE("DendoStepper", "Steps per millimeter not set, cannot move!");
    }
    return runPos(relative * ctrl.stepsPerMm, direction);
}

esp_err_t DendoStepper::runAbs(uint32_t position, bool direction)
{
    ...
    return runPos(relativeSteps, direction); // run to new position
}

esp_err_t DendoStepper::runAbsMm(uint32_t position, bool direction)
{
    if (ctrl.stepsPerMm == 0)
    {
        STEP_LOGE("DendoStepper", "Steps per millimeter not set, cannot move!");
    }
    return runAbs(position * ctrl.stepsPerMm, direction);
}
...
{
    ctrl.dir = state;
    gpio_set_level((gpio_num_t)conf.dirPin, state);
   
    //STEP_LOGI("DendoStepper", "Direction set: %d current state: %d ", state, gpio_get_level((gpio_num_t)conf.dirPin));
    //ESP_ERROR_CHECK(gpio_set_level((gpio_num_t)conf.dirPin, state));
}

bool DendoStepper::getDir()
{
    //ctrl.dir = state;
    //STEP_LOGI("DendoStepper", "Direction: %d ",ctrl.dir);
    return gpio_get_level((gpio_num_t)conf.dirPin);
}
